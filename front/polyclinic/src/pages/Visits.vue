...
const filtered = computed(() => {
  ...
  let list = visits.value.filter((v) => {
    ...
    const patient =
      `${v.patient.lastName} ${v.patient.firstName} ${v.patient.middleName || ''}`.toLowerCase()
...
<template>
  <div class="max-w-7xl mx-auto p-8 flex flex-col gap-10">
    <h1 class="text-4xl font-extrabold text-teal-800 tracking-tight">Все визиты</h1>
    ...
    <div v-else-if="filtered.length === 0" class="text-gray-500 text-lg text-center py-10">
      Нет визитов по выбранным параметрам
    </div>
    ...
    <div v-else class="grid grid-cols-[repeat(auto-fit,minmax(340px,1fr))] gap-8">
      <VisitCard
        ...
        :canEdit="canEditVisit(v)"
        :canDelete="canDeleteVisit(v)"
