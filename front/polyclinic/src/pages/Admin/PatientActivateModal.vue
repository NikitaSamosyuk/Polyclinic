...
async function confirm() {
  try {
    loading.value = true
    error.value = null
    await patientsApi.activate(props.patient.id)
    emit('updated')
  } catch (e: any) {
    error.value = e?.response?.data?.message || 'Ошибка активации'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white p-8 rounded-xl shadow-xl w-full max-w-md border border-teal-300">
      <h2 class="text-xl font-bold mb-4 text-teal-800">Активировать пациента?</h2>
