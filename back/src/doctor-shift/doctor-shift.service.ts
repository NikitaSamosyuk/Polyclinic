...
import { Cabinet, DoctorShift } from '@prisma/client'

type ShiftWithRelations = DoctorShift & { cabinet: Cabinet; doctor: any }
...
export class DoctorShiftService {
  ...
  private validateWithinCabinet(start: string, end: string, cabinet: Cabinet): void {
    ...
    if (s < cStart || e > cEnd) {
      throw new BadRequestException('Смена должна быть внутри рабочего времени кабинета')
    }
  }
  ...
  private async ensureNoOverlap(
    ...
    for (const s of doctorShifts) {
      ...
      if (start < sEnd && end > sStart) {
        throw new BadRequestException('Смена пересекается с другой сменой врача')
      }
    }
    ...
    for (const s of cabinetShifts) {
      ...
      const sEnd = this.timeToMinutes(s.endTime)
      if (start < sEnd && end > sStart) {
        throw new BadRequestException('Смена пересекается с другой сменой в кабинете')
      }
    }
  }
  ...
  async getMany(params: { doctorId?: number; cabinetId?: number; date?: string }) {
  ...
  async create(dto: CreateShiftDto): Promise<ShiftWithRelations> {
    ...
    await this.ensureNoOverlap(dto.doctorId, dto.cabinetId, date, dto.startTime, dto.endTime)
    ...
    return this.prisma.doctorShift.create({
      ...
      include: { cabinet: true, doctor: true }, // ← ВАЖНО
    })
  }

  async update(id: number, dto: UpdateShiftDto): Promise<ShiftWithRelations> {
    const existing = await this.prisma.doctorShift.findUnique({ where: { id } })
    ...
    await this.ensureNoOverlap(doctorId, cabinetId, date, startTime, endTime, id)
